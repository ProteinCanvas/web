...
export function ImportDropzone({ onImport }: ImportDropzoneProps) {
  ...
  const processFiles = useCallback(
    async (files: File[]) => {
      ...
      if (!adapter) {
        setError(
          'No compatible format detected. Supported: PDB/CSV (RFdiffusion), FASTA (ProteinMPNN), CIF/JSON (AlphaFold3), or any generic CSV.'
        )
        return
      }
      ...
      try {
        ...
        if (partial.candidates.length === 0) {
          setError(`${adapter.label}: no candidates were found in the provided files. Check that the files are complete and not empty.`)
          return
        }
        const enrichedCandidates = enrichWithDevMetrics(partial.candidates)
        const campaign: Campaign = {
          ...partial,
          candidates: enrichedCandidates,
          metricFields: inferMetricFields(enrichedCandidates),
          id: nanoid(),
        ...
        const raw = err instanceof Error ? err.message : 'Failed to parse campaign files.'
        const friendly = raw.includes('JSON')
          ? 'Could not parse JSON — make sure the file is valid and not corrupted.'
          : raw
        setError(friendly)
      }
    },
  ...
  const handleLoadSample = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Could not fetch sample (HTTP ${res.status}). The sample files may not be available in this deployment.`)
      ...
      if (
        ...
        throw new Error('Sample file has an unexpected format and cannot be loaded.')
      }
      setImportState('idle')
      onImport(data as Campaign)
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Failed to load sample campaign.'
      setError(msg)
    }
  }, [onImport, setError])
      ...
      try {
        const files = await Promise.all(
          paths.map(async (p) => {
            ...
            if (!res.ok) throw new Error(`Could not fetch ${p.split('/').pop()} (HTTP ${res.status}). The sample file may not be available.`)
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full">
      <div
        ...
        onDrop={handleDrop}
        className={[
          'relative w-full rounded-lg border-2 px-8 py-16 flex flex-col items-center justify-center gap-4 transition-all duration-150 cursor-pointer select-none',
          isDragging
      ...
      >
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-xs text-zinc-500 dark:text-zinc-400">
                Click or drop files to try again
              </p>
            </div>
          ) : (
            <>
        ...
        {detectedLabel && !isLoading && (
          <div className="absolute bottom-4 px-3 py-1 rounded bg-primary/10 border border-primary/30">
