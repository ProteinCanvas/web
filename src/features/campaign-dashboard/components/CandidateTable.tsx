...
export function CandidateTable({
  ...
  const campaignShortlist = useCampaignStore((s) =>
    s.shortlist.filter((e) => e.campaignId === campaignId)
  )
  ...
  const shortlistedSeqMap = useMemo(() => {
    const map = new Map<string, string>()
    const shortlistedIds = new Set(campaignShortlist.map((e) => e.candidateId))
    for (const c of candidates) {
      if (shortlistedIds.has(c.id) && c.sequence) map.set(c.id, c.sequence)
    }
    return map
  }, [candidates, campaignShortlist])

  const handleShortlistToggle = useCallback(
    (candidateId: string, starred: boolean, sequence?: string) => {
      if (starred) {
        removeFromShortlist(candidateId, campaignId)
        return
      }
      addToShortlist({ candidateId, campaignId, addedAt: new Date().toISOString(), status: 'candidate', notes: '' })
      if (!sequence) return
      let nearMatch: number | null = null
      shortlistedSeqMap.forEach((existingSeq, existingId) => {
        if (nearMatch !== null || existingId === candidateId) return
        const len = Math.min(existingSeq.length, sequence.length)
        let dist = Math.abs(existingSeq.length - sequence.length)
        for (let i = 0; i < len; i++) {
          if (existingSeq[i] !== sequence[i]) dist++
          if (dist > 3) break
        }
        if (dist <= 3) nearMatch = dist
      })
      if (nearMatch !== null) {
        toast(`Near-identical to a shortlisted candidate (${nearMatch} mutation${nearMatch === 1 ? '' : 's'})`, 'info')
      }
    },
    [campaignId, addToShortlist, removeFromShortlist, shortlistedSeqMap, toast]
  )
  ...
  useEffect(() => {
    ...
    applyFilter(sqlFilter).then((err) => {
      setSqlError(err)
      if (sqlFilter.trim()) setQueryTime(performance.now() - t0)
      else setQueryTime(null)
    })
    ...
    const metricCols: ColumnDef<Candidate>[] = visibleMetricFields.map((field) => ({
      ...
      header: () => (
        <div className="flex items-center gap-1 justify-end">
          ...
          <MetricInfo metricKey={field.key} placement="below" />
        </div>
      ),
          ...
          return (
            <div className="flex items-center gap-2">
              <button
                onClick={(e) => {
                  e.stopPropagation()
                  handleShortlistToggle(candidateId, starred, row.original.sequence ?? undefined)
                }}
        ...
        cell: ({ row, getValue }: { row: { original: Candidate }; getValue: () => unknown }) => (
          <div className="flex items-center justify-between gap-2">
            ...
            <button
              ...
              className="shrink-0 p-1 rounded text-muted-foreground hover:text-foreground hover:bg-muted transition-colors opacity-0 group-hover/row:opacity-100"
  ...
  }, [visibleMetricFields, campaignId, isShortlisted, handleShortlistToggle, addToComparison])
  ...
  return (
    <div className={`flex flex-col gap-2 min-h-0 ${className ?? ''}`} onKeyDown={handleKeyDown} tabIndex={0} role="grid" aria-label="Candidate table">
      ...
      <p className="text-xs text-muted-foreground flex items-center gap-1.5">
        ...
        {sqlFilter && queryTime !== null && (
          <span className="text-xs text-muted-foreground/50 font-mono">{queryTime.toFixed(0)}ms</span>
        )}
        {sqlFilter && <span className="text-primary">· SQL filter active</span>}
        {showShortlistOnly && <span className="text-amber-400">· Shortlisted only</span>}
      </p>
    </div>
  )
}
